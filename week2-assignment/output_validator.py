...
class OutputValidator:
    ...
    # Default allowed tools for customer support
    ...
    def __init__(self, allowed_emails: List[str] = None, allowed_tools: Set[str] = None):
        """
        Initialize validator.
        
        Args:
            allowed_emails: List of email addresses that are safe to include
            allowed_tools: Set of tool names that are allowed to be called
        """
    ...
    def check_tool_allowlist(self, tool_name: str) -> Tuple[bool, Optional[str]]:
        """
        Validate that tool being used is in allowlist.
        ...
        logger.error(f"Tool not in allowlist: {tool_name}")
        return False, f"Tool '{tool_name}' is not authorized for use"
    
    def extract_tool_calls(self, response_text: str) -> List[str]:
        """
        Extract potential tool calls from response text.
        Common patterns: [TOOL: tool_name], {{tool_name}}, etc.
    ...
    def validate_tool_usage(self, response_text: str) -> Tuple[bool, Optional[str]]:
        """
        Check if response attempts to use unauthorized tools.
        ...
        """
        detected_tools = self.extract_tool_calls(response_text)
        
        for tool in detected_tools:
            is_allowed, reason = self.check_tool_allowlist(tool)
            if not is_allowed:
                logger.error(f"Unauthorized tool usage detected: {tool}")
                return False, f"Unauthorized tool usage: {reason}"
